...
class VoucherSource(str, Enum):
    """Where a voucher came from. Only manual stock is ever printable."""
    manual = "manual"      # generated by the operator — unsold stock
    online = "online"      # created when a captive-portal payment succeeded
    reseller = "reseller"  # allocated to, and paid for by, a reseller
...
class VoucherResponse(BaseModel):
    ...
    source: VoucherSource = VoucherSource.manual
...
# --- Voucher printing ---
class PrintBatchPlan(BaseModel):
    """What a printed card shows about the plan."""
    id: uuid.UUID
    name: str
    type: str
    price_ghs: Decimal
...
class PrintBatchVoucher(BaseModel):
    """Only the code: it is all a customer types on the captive portal (the
    portal resolves username/password itself), so the print payload never
    carries credentials."""
    id: uuid.UUID
    code: str
    created_at: Optional[datetime] = None
...
class PrintBatchResponse(BaseModel):
    plan: PrintBatchPlan
    vouchers: list[PrintBatchVoucher]
    # Unprinted stock left for this plan in total, whether or not `limit`
    # truncated this response.
    available: int


class PrintConfirmRequest(BaseModel):
    voucher_ids: list[uuid.UUID] = Field(min_length=1)


class PrintConfirmResponse(BaseModel):
    confirmed: int
    # In the request but already marked printed — most likely a concurrent
    # print run of the same stock. The UI should warn: those cards may exist
    # twice on paper.
    already_printed: list[uuid.UUID]
    # Unknown to this operator, or no longer printable stock (sold online,
    # reseller, used, disabled). Never marked.
    not_printable: list[uuid.UUID]
